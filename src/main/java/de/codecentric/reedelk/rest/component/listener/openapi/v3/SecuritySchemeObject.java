...
public class SecuritySchemeObject implements Implementor, OpenAPIModel<de.codecentric.reedelk.openapi.v3.model.SecuritySchemeObject> {
    ...
    @Override
    public de.codecentric.reedelk.openapi.v3.model.SecuritySchemeObject map(OpenApiSerializableContext context) {
        de.codecentric.reedelk.openapi.v3.model.SecuritySchemeObject mapped =
                new de.codecentric.reedelk.openapi.v3.model.SecuritySchemeObject();
        mapped.setType(de.codecentric.reedelk.openapi.v3.model.SecurityType.valueOf(type.name()));
        mapped.setDescription(description);
        mapped.setName(name);
        if (in != null) {
            mapped.setIn(de.codecentric.reedelk.openapi.v3.model.SecurityKeyLocation.valueOf(in.name()));
        }
        mapped.setScheme(scheme);
        mapped.setBearerFormat(bearerFormat);
        if (flows != null) {
            de.codecentric.reedelk.openapi.v3.model.OAuthFlowsObject mappedFlows = flows.map(context);
