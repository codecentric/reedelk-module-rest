...
public class DefaultServerRoutes implements HttpServerRoutes {
    ...
    public void remove(HttpMethod method, String path) {
        handlers.stream()
                .filter(handler -> HttpPredicate.MatcherResult.EXACT_MATCH.equals(handler.matches(method, path)))
                .findFirst()
    ...
    private boolean isAlreadyRegistered(HttpMethod method, String path) {
        ...
        return handlers
                .stream()
                .anyMatch(handler -> HttpPredicate.MatcherResult.EXACT_MATCH.equals(handler.matches(method, path)));
    }
    ...
    static Optional<HttpRouteHandler> findMatchingHttpRouteHandler(List<HttpRouteHandler> handlers, HttpServerRequest request) {
        ...
        while (iterator.hasNext()) {
            ...
            HttpPredicate.MatcherResult templateMatch = handler.matches(request);
            if (HttpPredicate.MatcherResult.EXACT_MATCH.equals(templateMatch)) {
                // Priority goes to exact matches.
                return Optional.of(handler);
            } else if (HttpPredicate.MatcherResult.TEMPLATE_MATCH.equals(templateMatch)) {
    ...
    static class RouteAlreadyDefinedException extends PlatformException {
        ...
        RouteAlreadyDefinedException(String method, String path) {
            super(Messages.RestListener.ERROR_ROUTE_ALREADY_DEFINED.format(method, path));
        }
    }
}
