...
import static de.codecentric.reedelk.runtime.api.message.content.MimeType.UNKNOWN;
...
class RESTListenerGetTest extends RESTListenerAbstractTest {
    ...
    void shouldReturn200WhenBasePathOnlyIsDefined() {
        ...
        RESTListener listener = listenerWith(RestMethod.GET, configWithBasePath);
        listener.addEventListener((message, callback) -> callback.onResult(context, message));
        listener.onStart();
    ...
    void shouldCorrectlyMapHttpRequestToInboundMessageAttributes() throws IOException {
        ...
        assertThatAttributeIsEqualTo(attributes, RESTListenerAttributes.HEADERS, headers -> {
        ...
        assertThatAttributeIsEqualTo(attributes, RESTListenerAttributes.MATCHING_PATH, "/group/{groupId}");
        assertThatAttributeIsEqualTo(attributes, RESTListenerAttributes.METHOD, "GET");
        assertThatAttributeIsEqualTo(attributes, RESTListenerAttributes.PATH_PARAMS, new TreeMap<>(of("groupId", "managers")));
        assertThatAttributeIsEqualTo(attributes, RESTListenerAttributes.QUERY_PARAMS, new TreeMap<>(of("query1", singletonList("value1"), "query2", asList("value2", "value3"))));
        assertThatAttributeIsEqualTo(attributes, RESTListenerAttributes.QUERY_STRING, "query1=value1&query2=value2&query2=value3");
        assertThatAttributeIsEqualTo(attributes, RESTListenerAttributes.REQUEST_PATH, "/api/internal/group/managers");
        assertThatAttributeIsEqualTo(attributes, RESTListenerAttributes.REQUEST_URI, "/api/internal/group/managers?query1=value1&query2=value2&query2=value3");
        assertThatAttributeIsEqualTo(attributes, RESTListenerAttributes.SCHEME, "http");
        assertThatAttributeIsEqualTo(attributes, RESTListenerAttributes.VERSION, "HTTP/1.1");

        String remoteAddress = attributes.get(RESTListenerAttributes.REMOTE_ADDRESS);
