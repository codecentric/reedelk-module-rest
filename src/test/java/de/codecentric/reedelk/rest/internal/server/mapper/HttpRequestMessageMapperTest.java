...
class HttpRequestMessageMapperTest {
    ...
    void shouldCorrectlyMapMessageAttributesAndContentCorrectly() {
        ...
        assertThatContainsAttribute(message, RESTListenerAttributes.REMOTE_ADDRESS, "localhost/127.0.0.1:7070");
        assertThatContainsAttribute(message, RESTListenerAttributes.MATCHING_PATH, matchingPath);
        assertThatContainsAttribute(message, RESTListenerAttributes.QUERY_PARAMS, expectedQueryParams);
        assertThatContainsAttribute(message, RESTListenerAttributes.REQUEST_PATH, requestPath);
        assertThatContainsAttribute(message, RESTListenerAttributes.REQUEST_URI, requestUri);
        assertThatContainsAttribute(message, RESTListenerAttributes.QUERY_STRING, queryString);
        assertThatContainsAttribute(message, RESTListenerAttributes.PATH_PARAMS, params);
        assertThatContainsAttribute(message, RESTListenerAttributes.VERSION, HttpVersion.HTTP_1_0.text());
        assertThatContainsAttribute(message, RESTListenerAttributes.HEADERS, expectedHeaders);
        assertThatContainsAttribute(message, RESTListenerAttributes.SCHEME, HttpScheme.HTTP.toString());
        assertThatContainsAttribute(message, RESTListenerAttributes.METHOD, PUT.name());
